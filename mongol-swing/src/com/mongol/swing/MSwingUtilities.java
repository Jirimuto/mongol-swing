...
import com.mongol.swing.text.MView;
...
public class MSwingUtilities implements SwingConstants {
    /**
     ...
     */
    private static final StringBuilder SKIP_CLICK_COUNT = new StringBuilder(
            "skipClickCount");
     ...
     */
    public static final Object COMPONENT_UI_PROPERTY_KEY = new StringBuffer(
            "ComponentUIPropertyKey");

    /** Client Property key for the text maximal offsets for BasicMenuItemUI */
    public static final Object BASICMENUITEMUI_MAX_TEXT_OFFSET = new StringBuilder(
            "maxTextOffset");
...
//    		return true;
        ...
        int textIconGap)
    {
        ...
        if (textIsEmpty) {
            ...
            v = (c != null) ? (MView) c.getClientProperty("html") : null;
            if (v != null) {
                textR.width = Math.min(availTextWidth,
                                       (int) v.getPreferredSpan(View.X_AXIS));
                textR.height = (int) v.getPreferredSpan(View.Y_AXIS);
            } else {
                // textR.width = MSwingUtilities.stringWidth(c, fm, text);
            	Font font = MongolianFontUtil.getAltFont(fm.getFont(), null);
            	FontMetrics default_fm = c.getFontMetrics( font );
                Segment s = new Segment(text.toCharArray(), 0, text.length());
                
            	textR.width = MUtilities.getTabbedTextWidth(v, s, fm, default_fm, 0, null, 0, null);
                lsb = MSwingUtilities.getLeftSideBearing(c, fm, text);
                if (lsb < 0) {
                ...
                if (textR.width > availTextWidth) {
                    text = MSwingUtilities.clipString(c, fm, text,
                                                      availTextWidth);
                    s = new Segment(text.toCharArray(), 0, text.length());
                	textR.width = MUtilities.getTabbedTextWidth(v, s, default_fm, fm, 0, null, 0, null);
                    // textR.width = MSwingUtilities.stringWidth(c, fm, text);
                }
    ...
    public static int getRightSideBearing(JComponent c, FontMetrics fm,
        ...
        return getRightSideBearing(c, fm, string
    ...
    private static int getBearing(JComponent comp, FontMetrics fm,
        ...
        synchronized (MSwingUtilities.class) {
            ...
            if (entry == null) {
                Iterator<SoftReference<BearingCacheEntry>> iter = softBearingCache
                        .iterator();
                while (iter.hasNext()) {
            ...
            return (isLeftBearing) ? entry.getLeftSideBearing(c)
    ...
    private synchronized static void cacheEntry(BearingCacheEntry entry) {
        ...
        if (oldestEntry != null) {
            softBearingCache.add(new SoftReference<BearingCacheEntry>(
                    oldestEntry));
        }
    ...
    public static String clipStringIfNecessary(JComponent c,
        ...
        if (textWidth > availTextWidth) {
            return MSwingUtilities.clipString(c, fm, string,
                    availTextWidth);
        }
        return string;
    }
    ...
    public static String clipString(JComponent c, FontMetrics fm,
        ...
        availTextWidth -= MSwingUtilities
                .stringWidth(c, fm, clipString);
        boolean needsTextLayout = false;

        synchronized (charsBufferLock) {
            if (charsBuffer == null
                    || charsBuffer.length < stringLength) {
        ...
        if (needsTextLayout) {
            ...
            LineBreakMeasurer measurer = new LineBreakMeasurer(aString
                    .getIterator(), frc);
    ...
    public static void drawString(JComponent c, Graphics g,
    	...
    	drawString(c, g, text, x, y, MSwingRotateUtilities.ROTATE_HORIZANTAL, MSwingRotateUtilities.ROTATE_DEFAULT);
    }
    /**
     * Draws the string at the specified location.
     *
     ...
     * @param x X coordinate to draw the text at
     * @param y Y coordinate to draw the text at
     * @param rotate_direction Component Rotate Direction
     * @param rotate_hint Component Text Rotate Hint
     */
    public static void drawString(JComponent c, Graphics g,
            String text, int x, int y, int rotate_direction, int rotate_hint) {
        // c may be null
        ...
        if( rotate_direction == MSwingRotateUtilities.ROTATE_HORIZANTAL || rotate_hint == MSwingRotateUtilities.ROTATE_DEFAULT 
        		|| rotate_hint == MSwingRotateUtilities.ROTATE_LEFTTORIGHT || rotate_hint == MSwingRotateUtilities.ROTATE_RIGHTTOLEFT ) {
	        ...
	        if (isPrinting(g)) {
	            ...
	            if (g2d != null) {
	                ...
	                if (col instanceof  PrintColorUIResource) {
	                    g2d.setColor(((PrintColorUIResource) col)
	                            .getPrintColor());
	                }
	        ...
	        if (info != null && (g instanceof  Graphics2D)) {
	            ...
	            Object oldAAValue = g2
	                    .getRenderingHint(KEY_TEXT_ANTIALIASING);
	            ...
	            if (info.lcdContrastHint != null) {
	                oldContrast = g2
	                        .getRenderingHint(KEY_TEXT_LCD_CONTRAST);
        	...
        	// if the character is DEFAULT_CJK the rotate_hint will be ROTATE_NONE
        	// if rotate_direction == MSwingRotateUtilities.ROTATE_VERTICAL && rotate_hint == MSwingRotateUtilities.ROTATE_NONE then
        	// Graphic rotate back to normal.
    		g.setColor(c.getForeground());
    		g.setFont(c.getFont());
    		MSwingRotateUtilities rt = new MSwingRotateUtilities();
    		rt.calcDimensions(c, g, text, rotate_hint);
    		
    		if (rotate_hint == MSwingRotateUtilities.ROTATE_NONE) {
    			...
    			for (int i = 0; i < rt.fCharStrings.length; i++) {
    				...
    				switch (rt.fPosition[i]) {
    					case MSwingRotateUtilities.POSITION_NORMAL: 
    						...
    						break;
    					case MSwingRotateUtilities.POSITION_TOP_RIGHT:
    						tweak = rt.fCharHeight/3; // Should be 2, but they aren't actually half-height
    						g2d.drawString(rt.fCharStrings[i], xPos+(tweak/2), yPos-tweak); 
    						break;
    					case MSwingRotateUtilities.POSITION_FAR_TOP_RIGHT:
    						tweak = rt.fCharHeight - rt.fCharHeight/3;
    						g2d.drawString(rt.fCharStrings[i], xPos+(tweak/2), yPos-tweak); 
    						break;
    				}
    				yPos += rt.fCharHeight;
    ...
    public static void drawStringUnderlineCharAt(JComponent c,
    		...
    		drawStringUnderlineCharAt(c, g, text, underlinedIndex, x, y, 
    				MSwingRotateUtilities.ROTATE_HORIZANTAL, MSwingRotateUtilities.ROTATE_DEFAULT );
    }
     ...
     */
    public static void drawStringUnderlineCharAt(JComponent c,
            Graphics g, String text, int underlinedIndex, int x, int y, TabExpander e,int rotate_direction, int rotate_hint ) {
        ...
        Segment s = new Segment(text.toCharArray(), 0, text.length());
        int offset = MUtilities.drawTabbedText(c, s, x, y, g, e, 0, null);
        ...
        if (underlinedIndex >= 0 && underlinedIndex < textLength) {
            ...
            if (!needsTextLayout) {
                ...
                underlineRectX = x
                        + MSwingUtilities.stringWidth(c, fm, text
                                .substring(0, underlinedIndex));
                underlineRectWidth = fm.charWidth(text
                        .charAt(underlinedIndex));
            } else {
                Graphics2D g2d = getGraphics2D(g);
                if (g2d != null) {
                    TextLayout layout = new TextLayout(text, g2d
                            .getFont(), g2d.getFontRenderContext());
                    if (isPrinting) {
                        float screenWidth = (float) g2d.getFont()
                                .getStringBounds(text, DEFAULT_FRC)
                                .getWidth();
                        layout = layout.getJustifiedLayout(screenWidth);
                    }
                    TextHitInfo leading = TextHitInfo
                            .leading(underlinedIndex);
                    TextHitInfo trailing = TextHitInfo
                            .trailing(underlinedIndex);
                    Shape shape = layout.getVisualHighlightShape(
                            leading, trailing);
     ...
     */
    public static void drawStringUnderlineCharAt(JComponent c,
            Graphics g, String text, int underlinedIndex, int x, int y, int rotate_direction, int rotate_hint ) {
        if (text == null || text.length() <= 0) {
            return;
        }
    ...
    public static boolean pointOutsidePrefSize(JTable table, int row,
        ...
        Component cell = tcr.getTableCellRendererComponent(table,
    ...
    public static void setLeadAnchorWithoutSelection(
            ListSelectionModel model, int lead, int anchor) {
        if (anchor == -1) {
            anchor = lead;
        }
        if (lead == -1) {
     ...
     */
    public static int drawCharsWithFont(JComponent c, Graphics g, Font font, char[] data,
            int offset, int length, int x, int y) {
		Graphics2D g2 = (Graphics2D)g.create();
		g2.setFont(font);
    	int nextX = drawChars(c, g2, data, offset, length, x, y);
		g2.dispose();
		return nextX;
    }
    /**
    ...
    public static int drawChars(JComponent c, Graphics g, char[] data,
        ...
        // UIManager.getFont(c.getP + ".font");
        int nextX = x
                + getFontMetrics(c, g).charsWidth(data, offset, length);
        if (isPrinting(g)) {
            Graphics2D g2d = getGraphics2D(g);
            ...
            if (g2d != null) {
                ...
                if (frc != null
                    ...
                    TextLayout layout = new TextLayout(new String(data,
                            offset, length), g2d.getFont(),
                            deviceFontRenderContext);
                    float screenWidth = (float) g2d.getFont()
                            .getStringBounds(data, offset,
                                    offset + length, frc).getWidth();
                    if( screenWidth <= 0 )
                    	screenWidth = 1;
                    layout = layout.getJustifiedLayout(screenWidth);
    ...
    public static float drawString(JComponent c, Graphics g,
        ...
        if (isPrinting) {
            ...
            if (col instanceof  PrintColorUIResource) {
                g
                        .setColor(((PrintColorUIResource) col)
                                .getPrintColor());
            }
        }
        ...
        if (g2d == null) {
            ...
            if (isPrinting) {
                FontRenderContext deviceFRC = g2d
                        .getFontRenderContext();
     ...
     */
    private static boolean isFontRenderContextPrintCompatible(
            FontRenderContext frc1, FontRenderContext frc2) {
        ...
        if (frc1.getFractionalMetricsHint() != frc2
     ...
     */
    public static boolean useSelectedTextColor(Highlighter.Highlight h,
            JTextComponent c) {
        ...
        if (painterClass.indexOf("javax.swing.text.DefaultHighlighter") != 0
                && painterClass
                        .indexOf("com.sun.java.swing.plaf.windows.WindowsTextUI") != 0) {
            return false;
        }
        try {
    ...
    private static class BearingCacheEntry {
        ...
        private short calcBearing(char aChar) {
            ...
            Rectangle pixelBounds = gv.getGlyphPixelBounds(0, frc, 0f,
                    0f);
    ...
    private static synchronized boolean inputEvent_canAccessSystemClipboard(
        ...
        if (inputEvent_CanAccessSystemClipboard_Field == null) {
            inputEvent_CanAccessSystemClipboard_Field = (Field) AccessController
     ...
     * @param checkGesture boolean 
     */
    private static boolean canCurrentEventAccessSystemClipboard(
            boolean checkGesture) {
    ...
    public static Object makeIcon(final Class<?> baseClass,
            final Class<?> rootClass, final String imageFile) {
        ...
        return new UIDefaults.LazyValue() {
            public Object createValue(UIDefaults table) {
                 ...
                 */
                byte[] buffer = (byte[]) java.security.AccessController
                        .doPrivileged(new java.security.PrivilegedAction() {
                            public Object run() {
                                try {
    ...
    public static boolean isLocalDisplay() {
        try {
            ...
            Class x11Class = Class
                    .forName("sun.awt.X11GraphicsEnvironment");
            Method isDisplayLocalMethod = x11Class.getMethod(
                    "isDisplayLocal", new Class[0]);
            return (Boolean) isDisplayLocalMethod.invoke(null,
                    (Object[]) null);
        } catch (Throwable t) {
        }
    ...
    public static void setSkipClickCount(Component comp, int count) {
        if (comp instanceof  JTextComponent
            ...
            ((JTextComponent) comp).putClientProperty(SKIP_CLICK_COUNT,
                    count);
        }
    }
    ...
    public static int getAdjustedClickCount(JTextComponent comp,
        ...
        if (cc == 1) {
            ...
            Integer sub = (Integer) comp
                    .getClientProperty(SKIP_CLICK_COUNT);
            if (sub != null) {
                return cc - sub;
            }
        }
     ...
     *         {@code null}
     */
    private static Section liesIn(Rectangle rect, Point p,
            boolean horizontal, boolean ltr, boolean three) {
        ...
        if (three) {
            ...
            if (forward) {
                return pComp >= middle ? Section.TRAILING
                        : Section.LEADING;
            } else {
                return pComp < middle ? Section.TRAILING
                        : Section.LEADING;
