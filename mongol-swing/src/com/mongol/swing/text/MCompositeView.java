 ...
 */
public abstract class MCompositeView extends MView {
     ...
     */
    public MCompositeView(Element elem) {
        super(elem);
        children = new MView[1];
        nchildren = 0;
    ...
    public void replace(int offset, int length, View[] views) {
        ...
        Set<View> set = new HashSet<View>(Arrays.asList(views));
        // update parent reference on removed views
        for (int i = offset; i < offset + length; i++) {
            MView child = children[i];
        ...
        if ((nchildren + delta) >= children.length) {
            ...
            MView[] newChildren = new MView[newLength];
     ...
     */
    protected abstract MView getViewAtPoint(int x, int y, Rectangle alloc);
    ...
    protected int getNextNorthSouthVisualPositionFrom(int pos, Position.Bias b,
        ...
        return MUtilities.getNextVisualPositionFrom(
                            this, pos, b, a, direction, biasRet, getRotateDirection(), getRotateHint());
    }
    ...
    * @param pos the position to convert &gt;= 0
    ...
    private static MView[] ZERO = new MView[0];

    private MView[] children;
