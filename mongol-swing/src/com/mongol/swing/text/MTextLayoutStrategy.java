 ...
 */
public class MTextLayoutStrategy extends MFlowView.FlowStrategy {

    /**
     * Constructs a layout strategy for paragraphs based
     * upon java.awt.font.LineBreakMeasurer.
     */
    public MTextLayoutStrategy() {
     ...
     */
    protected int layoutRow(MFlowView fv, int rowIndex, int p0) {
    ...
    protected View createView(MFlowView fv, int startOffset, int spanLeft, int rowIndex) {
        ...
        if ((frag instanceof MGlyphView) && (measurer != null)) {
            ...
            if ((p1 - p0) == 1) {
                // check for tab
                Segment s = ((MGlyphView)frag).getText(p0, p1);
                char ch = s.first();
                if (ch == '\t') {
                    isTab = true;
                }
            ...
            TextLayout tl = (isTab) ? null :
                measurer.nextLayout(spanLeft, text.toIteratorIndex(endOffset),
                                    requireNextWord);
            if (tl != null) {
                ((MGlyphView)frag).setGlyphPainter(new MGlyphPainter2(tl));
            }
        }
        return frag;
    }
    ...
    void sync(MFlowView fv) {
        ...
        // TODO : Jirimuto added comment on the following line 
        // for disable TextLayout on MEditorPane --2020/07/18
        ...
        if( measurer != null ){
	        ...
	        for( int i=0; i<n; i++ ) {
	            ...
	            if( child instanceof MGlyphView ) {
	                ...
	                ((MGlyphView)child).setGlyphPainter(new MGlyphPainter2(layout));
	            }
	        }
    ...
    static class AttributedSegment extends Segment implements AttributedCharacterIterator {
        ...
        Font getFont(int childIndex) {
            ...
            if (child instanceof MGlyphView) {
                return ((MGlyphView)child).getFont();
            }
            return null;
        }
