...
/*
 * Modified AccessibleHTML 
 * 
 * @author  jrmt@Almas
 */
class MAccessibleHTML implements Accessible {
	...
	public MAccessibleHTML(JEditorPane pane) {
		editor = pane;
	...
	ElementInfo createElementInfo(Element e, ElementInfo parent) {
		...
		if (attrs != null) {
			...
			if (name == MHTML.Tag.IMG) {
				return new IconElementInfo(e, parent);
			} else if (name == MHTML.Tag.CONTENT || name == MHTML.Tag.CAPTION) {
				return new TextElementInfo(e, parent);
			} else if (name == MHTML.Tag.TABLE) {
	...
	class TextElementInfo extends ElementInfo implements Accessible {
		...
		public class TextAccessibleContext extends HTMLAccessibleContext
			 ...
			 * distinguish between sublasses that behave differently (e.g.,
	...
	private class IconElementInfo extends ElementInfo implements Accessible {
		...
		private int getImageSize(Object key) {
			if (validateIfNecessary()) {
				...
				if (size == -1) {
					...
					size = 0;
					if (v instanceof MImageView) {
						Image img = ((MImageView) v).getImage();
						if (img != null) {
							if (key == MHTML.Attribute.WIDTH) {
		...
		protected class IconAccessibleContext extends HTMLAccessibleContext
			...
			public String getAccessibleIconDescription() {
				return ((MImageView) getView()).getAltText();
			}
			...
			public int getAccessibleIconWidth() {
				if (width == -1) {
					width = getImageSize(MHTML.Attribute.WIDTH);
				}
				return width;
			}
			...
			public int getAccessibleIconHeight() {
				if (height == -1) {
					height = getImageSize(MHTML.Attribute.HEIGHT);
				}
				return height;
			}
		}
	...
	private class TableElementInfo extends ElementInfo implements Accessible {
		...
		protected void loadChildren(Element e) {
			...
			for (int counter = 0; counter < e.getElementCount(); counter++) {
				...
				if (attrs.getAttribute(StyleConstants.NameAttribute) == MHTML.Tag.TR) {
					addChild(new TableRowElementInfo(child, this, counter));

				} else if (attrs.getAttribute(StyleConstants.NameAttribute) == MHTML.Tag.CAPTION) {
		...
		private class TableRowElementInfo extends ElementInfo {
			...
			protected void loadChildren(Element e) {
				for (int x = 0; x < e.getElementCount(); x++) {
					...
					if (attrs.getAttribute(StyleConstants.NameAttribute) == MHTML.Tag.TH) {
					...
					} else if (attrs.getAttribute(StyleConstants.NameAttribute) == MHTML.Tag.TD) {
						addChild(new TableCellElementInfo(e.getElement(x),
								this, false));
					}
				}
			}
		...
		private class TableCellElementInfo extends ElementInfo {
			...
			public int getRowCount() {
				if (validateIfNecessary()) {
					return Math.max(
							1,
							getIntAttr(getAttributes(), MHTML.Attribute.ROWSPAN,
									1));
				}
				return 0;
			}
			...
			public int getColumnCount() {
				if (validateIfNecessary()) {
					return Math.max(
							1,
							getIntAttr(getAttributes(), MHTML.Attribute.COLSPAN,
									1));
				}
				return 0;
			}
			...
			if (getChildCount() > 0) {
				...
				if (e.getType() != DocumentEvent.EventType.REMOVE) {
					...
					while ((index1 + 1) < getChildCount()
							&& getChild(index1 + 1).getElement().getEndOffset() == getChild(
									index1 + 1).getElement().getStartOffset()) {
						index1++;
					}
				}
