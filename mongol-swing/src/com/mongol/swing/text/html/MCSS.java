 ...
 * @author  jrmt@Almas
 */
public class MCSS implements Serializable {
	...
	valueConvertor.put(MCSS.Attribute.FONT_SIZE, new FontSize());
	valueConvertor.put(MCSS.Attribute.FONT_FAMILY, new FontFamily());
	valueConvertor.put(MCSS.Attribute.FONT_WEIGHT, new FontWeight());
	valueConvertor.put(MCSS.Attribute.BORDER_STYLE, new BorderStyle());
	Object cv = new ColorValue();
	valueConvertor.put(MCSS.Attribute.COLOR, cv);
	valueConvertor.put(MCSS.Attribute.BACKGROUND_COLOR, cv);
	valueConvertor.put(MCSS.Attribute.BORDER_COLOR, cv);
	Object lv = new LengthValue();
	valueConvertor.put(MCSS.Attribute.MARGIN_TOP, lv);
	valueConvertor.put(MCSS.Attribute.MARGIN_BOTTOM, lv);
	valueConvertor.put(MCSS.Attribute.MARGIN_LEFT, lv);
	valueConvertor.put(MCSS.Attribute.MARGIN_LEFT_LTR, lv);
	valueConvertor.put(MCSS.Attribute.MARGIN_LEFT_RTL, lv);
	valueConvertor.put(MCSS.Attribute.MARGIN_RIGHT, lv);
	valueConvertor.put(MCSS.Attribute.MARGIN_RIGHT_LTR, lv);
	valueConvertor.put(MCSS.Attribute.MARGIN_RIGHT_RTL, lv);
	valueConvertor.put(MCSS.Attribute.PADDING_TOP, lv);
	valueConvertor.put(MCSS.Attribute.PADDING_BOTTOM, lv);
	valueConvertor.put(MCSS.Attribute.PADDING_LEFT, lv);
	valueConvertor.put(MCSS.Attribute.PADDING_RIGHT, lv);
	Object bv = new BorderWidthValue(null, 0);
	valueConvertor.put(MCSS.Attribute.BORDER_WIDTH, lv);
	valueConvertor.put(MCSS.Attribute.BORDER_TOP_WIDTH, bv);
	valueConvertor.put(MCSS.Attribute.BORDER_BOTTOM_WIDTH, bv);
	valueConvertor.put(MCSS.Attribute.BORDER_LEFT_WIDTH, bv);
	valueConvertor.put(MCSS.Attribute.BORDER_RIGHT_WIDTH, bv);
	Object nlv = new LengthValue(true);
	valueConvertor.put(MCSS.Attribute.TEXT_INDENT, nlv);
	valueConvertor.put(MCSS.Attribute.WIDTH, lv);
	valueConvertor.put(MCSS.Attribute.HEIGHT, lv);
	valueConvertor.put(MCSS.Attribute.BORDER_SPACING, lv);
	Object sv = new StringValue();
	valueConvertor.put(MCSS.Attribute.FONT_STYLE, sv);
	valueConvertor.put(MCSS.Attribute.TEXT_DECORATION, sv);
	valueConvertor.put(MCSS.Attribute.TEXT_ALIGN, sv);
	valueConvertor.put(MCSS.Attribute.VERTICAL_ALIGN, sv);
	Object valueMapper = new CssValueMapper();
	valueConvertor.put(MCSS.Attribute.LIST_STYLE_TYPE,
			   valueMapper);
	valueConvertor.put(MCSS.Attribute.BACKGROUND_IMAGE,
			   new BackgroundImage());
	valueConvertor.put(MCSS.Attribute.BACKGROUND_POSITION,
			   new BackgroundPosition());
	valueConvertor.put(MCSS.Attribute.BACKGROUND_REPEAT,
			   valueMapper);
	valueConvertor.put(MCSS.Attribute.BACKGROUND_ATTACHMENT,
			   valueMapper);
	Object generic = new CssValue();
	int n = MCSS.Attribute.allAttributes.length;
	for (int i = 0; i < n; i++) {
	    MCSS.Attribute key = MCSS.Attribute.allAttributes[i];
    ...
    void addInternalCSSValue(MutableAttributeSet attr,
			     MCSS.Attribute key, String value) {
	if (key == MCSS.Attribute.FONT) {
	    ShorthandFontParser.parseShorthandFont(this, value, attr);
	}
	else if (key == MCSS.Attribute.BACKGROUND) {
	    ShorthandBackgroundParser.parseShorthandBackground
		               (this, value, attr);
	}
	else if (key == MCSS.Attribute.MARGIN) {
	    ShorthandMarginParser.parseShorthandMargin(this, value, attr,
					   MCSS.Attribute.ALL_MARGINS);
	}
	else if (key == MCSS.Attribute.PADDING) {
	    ShorthandMarginParser.parseShorthandMargin(this, value, attr,
					   MCSS.Attribute.ALL_PADDING);
	}
	else if (key == MCSS.Attribute.BORDER_WIDTH) {
	    ShorthandMarginParser.parseShorthandMargin(this, value, attr,
					   MCSS.Attribute.ALL_BORDER_WIDTHS);
	}
	else {
     ...
     * strings are valid.
     */
    Object getInternalCSSValue(MCSS.Attribute key, String value) {
     ...
     */
    Font getFont(StyleContext sc, AttributeSet a, int defaultSize, MStyleSheet ss) {
        ss = getStyleSheet(ss);
    ...
    static int getFontSize(AttributeSet attr, int defaultSize, MStyleSheet ss) {
	// PENDING(prinz) this is a 1.1 based implementation, need to also
	// have a 1.2 version.
	FontSize sizeValue = (FontSize)attr.getAttribute(MCSS.Attribute.
							 FONT_SIZE);
     ...
     * @return the color
     */
    Color getColor(AttributeSet a, MCSS.Attribute key) {
     ...
     */
    float getPointSize(String size, MStyleSheet ss) {
     ...
     * key <code>key</code>.
     */
    float getLength(AttributeSet a, MCSS.Attribute key, MStyleSheet ss) {
        ss = getStyleSheet(ss);
	...
	MHTML.Tag tag = getHTMLTag(htmlAttrSet);
	if ((tag == MHTML.Tag.TD) || (tag == MHTML.Tag.TH)) {
	    // translate border width into the cells
	    AttributeSet tableAttr = elem.getParentElement().
		                     getParentElement().getAttributes();
	    translateAttribute(MHTML.Attribute.BORDER, tableAttr, cssAttrSet);
	    String pad = (String)tableAttr.getAttribute(MHTML.Attribute.CELLPADDING);
	    if (pad != null) {
		LengthValue v = 
		    (LengthValue)getInternalCSSValue(MCSS.Attribute.PADDING_TOP, pad);
		v.span = (v.span < 0) ? 0 : v.span;
		cssAttrSet.addAttribute(MCSS.Attribute.PADDING_TOP, v);
		cssAttrSet.addAttribute(MCSS.Attribute.PADDING_BOTTOM, v);
		cssAttrSet.addAttribute(MCSS.Attribute.PADDING_LEFT, v);
		cssAttrSet.addAttribute(MCSS.Attribute.PADDING_RIGHT, v);
	    }
	}
	if (elem.isLeaf()) {
	...
	if (tag == MHTML.Tag.CAPTION) {
	    /* 
	     * Navigator uses ALIGN for caption placement and IE uses VALIGN.
	     */
	    Object v = htmlAttrSet.getAttribute(MHTML.Attribute.ALIGN);
	    if ((v != null) && (v.equals("top") || v.equals("bottom"))) {
		cssAttrSet.addAttribute(MCSS.Attribute.CAPTION_SIDE, v);
		cssAttrSet.removeAttribute(MCSS.Attribute.TEXT_ALIGN);
	    } else {
		v = htmlAttrSet.getAttribute(MHTML.Attribute.VALIGN);
		if (v != null) {
		    cssAttrSet.addAttribute(MCSS.Attribute.CAPTION_SIDE, v);
		}
	    }
     ...
     * attributes for example MHTML.Attribute.VSPACE or
     * MHTML.Attribute.HSPACE, end up mapping to two CSS.Attribute's.
     * Therefore, the value associated with each MHTML.Attribute.
	...
	htmlAttrToCssAttrMap.put(MHTML.Attribute.COLOR,
				 new MCSS.Attribute[]{MCSS.Attribute.COLOR});
	htmlAttrToCssAttrMap.put(MHTML.Attribute.TEXT,
				 new MCSS.Attribute[]{MCSS.Attribute.COLOR});
	htmlAttrToCssAttrMap.put(MHTML.Attribute.CLEAR,
				 new MCSS.Attribute[]{MCSS.Attribute.CLEAR});
	htmlAttrToCssAttrMap.put(MHTML.Attribute.BACKGROUND,
				 new MCSS.Attribute[]{MCSS.Attribute.BACKGROUND_IMAGE});
	htmlAttrToCssAttrMap.put(MHTML.Attribute.BGCOLOR,
				 new MCSS.Attribute[]{MCSS.Attribute.BACKGROUND_COLOR});
	htmlAttrToCssAttrMap.put(MHTML.Attribute.WIDTH,
				 new MCSS.Attribute[]{MCSS.Attribute.WIDTH});
	htmlAttrToCssAttrMap.put(MHTML.Attribute.HEIGHT,
				 new MCSS.Attribute[]{MCSS.Attribute.HEIGHT});
	htmlAttrToCssAttrMap.put(MHTML.Attribute.BORDER,
				 new MCSS.Attribute[]{MCSS.Attribute.BORDER_TOP_WIDTH, MCSS.Attribute.BORDER_RIGHT_WIDTH, MCSS.Attribute.BORDER_BOTTOM_WIDTH, MCSS.Attribute.BORDER_LEFT_WIDTH});
	htmlAttrToCssAttrMap.put(MHTML.Attribute.CELLPADDING,
				 new MCSS.Attribute[]{MCSS.Attribute.PADDING});
	htmlAttrToCssAttrMap.put(MHTML.Attribute.CELLSPACING,
				 new MCSS.Attribute[]{MCSS.Attribute.BORDER_SPACING});
	htmlAttrToCssAttrMap.put(MHTML.Attribute.MARGINWIDTH,
				 new MCSS.Attribute[]{MCSS.Attribute.MARGIN_LEFT,
						     MCSS.Attribute.MARGIN_RIGHT});
	htmlAttrToCssAttrMap.put(MHTML.Attribute.MARGINHEIGHT,
				 new MCSS.Attribute[]{MCSS.Attribute.MARGIN_TOP,
						     MCSS.Attribute.MARGIN_BOTTOM});
	htmlAttrToCssAttrMap.put(MHTML.Attribute.HSPACE,
				 new MCSS.Attribute[]{MCSS.Attribute.PADDING_LEFT,
						     MCSS.Attribute.PADDING_RIGHT});
	htmlAttrToCssAttrMap.put(MHTML.Attribute.VSPACE,
				 new MCSS.Attribute[]{MCSS.Attribute.PADDING_BOTTOM,
						     MCSS.Attribute.PADDING_TOP});
	htmlAttrToCssAttrMap.put(MHTML.Attribute.FACE,
				 new MCSS.Attribute[]{MCSS.Attribute.FONT_FAMILY});
	htmlAttrToCssAttrMap.put(MHTML.Attribute.SIZE,
				 new MCSS.Attribute[]{MCSS.Attribute.FONT_SIZE});
	htmlAttrToCssAttrMap.put(MHTML.Attribute.VALIGN,
				 new MCSS.Attribute[]{MCSS.Attribute.VERTICAL_ALIGN});
	htmlAttrToCssAttrMap.put(MHTML.Attribute.ALIGN,
				 new MCSS.Attribute[]{MCSS.Attribute.VERTICAL_ALIGN,
						     MCSS.Attribute.TEXT_ALIGN,
						     MCSS.Attribute.FLOAT});
	htmlAttrToCssAttrMap.put(MHTML.Attribute.TYPE,
				 new MCSS.Attribute[]{MCSS.Attribute.LIST_STYLE_TYPE});
	htmlAttrToCssAttrMap.put(MHTML.Attribute.NOWRAP,
				 new MCSS.Attribute[]{MCSS.Attribute.WHITE_SPACE});
	...
	// HTML->CSS
	htmlValueToCssValueMap.put("disc", MCSS.Value.DISC);
	htmlValueToCssValueMap.put("square", MCSS.Value.SQUARE);
	htmlValueToCssValueMap.put("circle", MCSS.Value.CIRCLE);
	htmlValueToCssValueMap.put("1", MCSS.Value.DECIMAL);
	htmlValueToCssValueMap.put("a", MCSS.Value.LOWER_ALPHA);
	htmlValueToCssValueMap.put("A", MCSS.Value.UPPER_ALPHA);
	htmlValueToCssValueMap.put("i", MCSS.Value.LOWER_ROMAN);
	htmlValueToCssValueMap.put("I", MCSS.Value.UPPER_ROMAN);

	// CSS-> internal CSS
	cssValueToInternalValueMap.put("none", MCSS.Value.NONE);
	cssValueToInternalValueMap.put("disc", MCSS.Value.DISC);
	cssValueToInternalValueMap.put("square", MCSS.Value.SQUARE);
	cssValueToInternalValueMap.put("circle", MCSS.Value.CIRCLE);
	cssValueToInternalValueMap.put("decimal", MCSS.Value.DECIMAL);
	cssValueToInternalValueMap.put("lower-roman", MCSS.Value.LOWER_ROMAN);
	cssValueToInternalValueMap.put("upper-roman", MCSS.Value.UPPER_ROMAN);
	cssValueToInternalValueMap.put("lower-alpha", MCSS.Value.LOWER_ALPHA);
	cssValueToInternalValueMap.put("upper-alpha", MCSS.Value.UPPER_ALPHA);
	cssValueToInternalValueMap.put("repeat", MCSS.Value.BACKGROUND_REPEAT);
	cssValueToInternalValueMap.put("no-repeat",
				       MCSS.Value.BACKGROUND_NO_REPEAT);
	cssValueToInternalValueMap.put("repeat-x",
				       MCSS.Value.BACKGROUND_REPEAT_X);
	cssValueToInternalValueMap.put("repeat-y",
				       MCSS.Value.BACKGROUND_REPEAT_Y);
	cssValueToInternalValueMap.put("scroll",
				       MCSS.Value.BACKGROUND_SCROLL);
	cssValueToInternalValueMap.put("fixed",
				       MCSS.Value.BACKGROUND_FIXED);

        // Register all the CSS attribute keys for archival/unarchival
	Object[] keys = MCSS.Attribute.allAttributes;
	try {
    ...
    static int getIndexOfSize(float pt, MStyleSheet ss) {
        int[] sizeMap = (ss != null) ? ss.getSizeMap() : 
            MStyleSheet.sizeMapDefault;
     ...
     * are 1-7.
     */
    float getPointSize(int index, MStyleSheet ss) {
        ss = getStyleSheet(ss);
        int[] sizeMap = (ss != null) ? ss.getSizeMap() : 
            MStyleSheet.sizeMapDefault;
        --index;
	...
	if (htmlAttrSet.getAttribute(StyleConstants.NameAttribute) ==
	    ...
	    translateAttributes(MHTML.Tag.HR, htmlAttrSet, cssAttrSet);
	}
	while (keys.hasMoreElements()) {
	    Object key = keys.nextElement();
	    if (key instanceof MHTML.Tag) {
		MHTML.Tag tag = (MHTML.Tag)key;
		Object o = htmlAttrSet.getAttribute(tag);
		if (o != null && o instanceof AttributeSet) {
		    translateAttributes(tag, (AttributeSet)o, cssAttrSet);
		}
	    } else if (key instanceof MCSS.Attribute) {
		cssAttrSet.addAttribute(key, htmlAttrSet.getAttribute(key));
	    }
	}
    }

    private void translateAttributes(MHTML.Tag tag,
	...
	while (names.hasMoreElements()) {
	    ...
	    if (name instanceof MHTML.Attribute) {
		MHTML.Attribute key = (MHTML.Attribute)name;

		/*
		 * MHTML.Attribute.ALIGN needs special processing.
		 ...
		 */
		if (key == MHTML.Attribute.ALIGN) {
		    String htmlAttrValue = (String)htmlAttrSet.getAttribute(MHTML.Attribute.ALIGN);
		    if (htmlAttrValue != null) {
			MCSS.Attribute cssAttr = getCssAlignAttribute(tag, htmlAttrSet);
			if (cssAttr != null) {
		    ...
		    if (key == MHTML.Attribute.SIZE && !isHTMLFontTag(tag)) {
			continue;
		    }

		    translateAttribute(key, htmlAttrSet, cssAttrSet);
		}
	    } else if (name instanceof MCSS.Attribute) {
		cssAttrSet.addAttribute(name, htmlAttrSet.getAttribute(name));
	    }
	}
    }

    private void translateAttribute(MHTML.Attribute key,
					   AttributeSet htmlAttrSet,
					   MutableAttributeSet cssAttrSet) {
	/*
	 * In the case of all remaining MHTML.Attribute's they
	 * map to 1 or more CCS.Attribute.
	 */
	MCSS.Attribute[] cssAttrList = getCssAttribute(key);
    ...
    /**
     * Given a CSS.Attribute object and its corresponding MHTML.Attribute's
     * value, this method returns a CssValue object to associate with the
     * CSS attribute.
     *
     * @param the CSS.Attribute
     * @param a String containing the value associated MHTML.Attribtue.
     */
    Object getCssValue(MCSS.Attribute cssAttr, String htmlAttrValue) {
    ...
    /**
     * Maps an MHTML.Attribute object to its appropriate CSS.Attributes.
     *
     * @param MHTML.Attribute
     * @return CSS.Attribute[]
     */
    private MCSS.Attribute[] getCssAttribute(MHTML.Attribute hAttr) {
	return (MCSS.Attribute[])htmlAttrToCssAttrMap.get(hAttr);
    }

    /**
     * Maps MHTML.Attribute.ALIGN to either:
     ...
     * @return CSS.Attribute mapping for MHTML.Attribute.ALIGN.
     */
    private MCSS.Attribute getCssAlignAttribute(MHTML.Tag tag,
						   AttributeSet htmlAttrSet) {
	return MCSS.Attribute.TEXT_ALIGN;
/*
	String htmlAttrValue = (String)htmlAttrSet.getAttribute(MHTML.Attribute.ALIGN);
	...
	*/
     ...
     * @return MHTML.Tag
     */
    private MHTML.Tag getHTMLTag(AttributeSet htmlAttrSet) {
    ...
    private boolean isHTMLFontTag(MHTML.Tag tag) {
	return (tag != null && ((tag == MHTML.Tag.FONT) || (tag == MHTML.Tag.BASEFONT)));
    }
     ...
     * MHTML.Attribute to a CSS.Attribute, and then calls
	 ...
	 */
	int getValue(AttributeSet a, MStyleSheet ss) {
            ss = getStyleSheet(ss);
	    if (index) {
	...
	Object fromStyleConstants(StyleConstants key, Object value) {
            if (value instanceof Number) {
                ...
                fs.value = getIndexOfSize(((Number)value).floatValue(), MStyleSheet.sizeMapDefault);
	...
	Object parseCssValue(String value) {
	    MCSS.Value cssv = MCSS.getValue(value);
	    if (cssv != null) {
		if ((cssv == MCSS.Value.INSET) ||
		    (cssv == MCSS.Value.OUTSET) ||
		    (cssv == MCSS.Value.NONE) ||
		    (cssv == MCSS.Value.DOTTED) ||
		    (cssv == MCSS.Value.DASHED) ||
		    (cssv == MCSS.Value.SOLID) ||
		    (cssv == MCSS.Value.DOUBLE) ||
		    (cssv == MCSS.Value.GROOVE) ||
		    (cssv == MCSS.Value.RIDGE)) {
	...
	private void readObject(ObjectInputStream s)
	    ...
	    if (value != null) {
		style = MCSS.getValue((String)value);
	    }
	}

	// CSS.Values are static, don't archive it.
	transient private MCSS.Value style;
    }
	...
	Object parseHtmlValue(String value) {
	    if (value.equals(MHTML.NULL_ATTRIBUTE_VALUE)) {
		value = "1";
	    }
	    return parseCssValue(value);
	}
	...
	Object parseHtmlValue(String value) {
	    if (value == MHTML.NULL_ATTRIBUTE_VALUE) {
	...
	Object parseCssValue(String value) {
	    ...
	    String[]  strings = MCSS.parseStrings(value);
	...
	ImageIcon getImage(URL base) {
		...
		synchronized(this) {
		    if (!loadedImage) {
			URL url = MCSS.getURL(base, svalue);
			loadedImage = true;
			if (url != null) {
                            image = new ImageIcon();
	 ...
	 */
	static void parseShorthandFont(MCSS css, String value,
		...
		if ((found & 1) == 0 && isFontStyle(strings[index])) {
		    css.addInternalCSSValue(attr, MCSS.Attribute.FONT_STYLE,
					    strings[index++]);
		    found |= 1;
		}
		else if ((found & 2) == 0 && isFontVariant(strings[index])) {
		    css.addInternalCSSValue(attr, MCSS.Attribute.FONT_VARIANT,
					    strings[index++]);
		    found |= 2;
		}
		else if ((found & 4) == 0 && isFontWeight(strings[index])) {
		    css.addInternalCSSValue(attr, MCSS.Attribute.FONT_WEIGHT,
					    strings[index++]);
		    found |= 4;
		}
		...
		css.addInternalCSSValue(attr, MCSS.Attribute.FONT_SIZE,
					fontSize);
	    }
	    else {
		css.addInternalCSSValue(attr, MCSS.Attribute.FONT_SIZE,
		...
		// line height
		if (lineHeight != null) {
		    css.addInternalCSSValue(attr, MCSS.Attribute.LINE_HEIGHT,
					    lineHeight);
		}
		else {
		    css.addInternalCSSValue(attr, MCSS.Attribute.LINE_HEIGHT,
		...
		css.addInternalCSSValue(attr, MCSS.Attribute.FONT_FAMILY,
					family);
	    }
	    else {
		css.addInternalCSSValue(attr, MCSS.Attribute.FONT_FAMILY,
	 ...
	 * result in <code>attr</code>.
	 */
	static void parseShorthandBackground(MCSS css, String value,
					     MutableAttributeSet attr) {
		...
		if ((found & 1) == 0 && isImage(string)) {
		    css.addInternalCSSValue(attr, MCSS.Attribute.
	    ...
	    if ((found & 1) == 0) {
		css.addInternalCSSValue(attr, MCSS.Attribute.BACKGROUND_IMAGE,
					null);
	    }
	    if ((found & 2) == 0) {
		css.addInternalCSSValue(attr, MCSS.Attribute.BACKGROUND_REPEAT,
					"repeat");
	    }
	    if ((found & 4) == 0) {
		css.addInternalCSSValue(attr, MCSS.Attribute.
	...
	static boolean isColor(String string) {
	    return (MCSS.stringToColor(string) != null);
	}
    }
	 ...
	 */
	static void parseShorthandMargin(MCSS css, String value,
					 MutableAttributeSet attr,
					 MCSS.Attribute[] names) {
	    String[] strings = parseStrings(value);
	    int count = strings.length;
	    int index = 0;
	    switch (count) {
     ...
     * idk 08/30/2004
     */
    private MStyleSheet getStyleSheet(MStyleSheet ss) {
        if (ss != null) {
            styleSheet = ss;
        }
        return styleSheet;
    ...
    private transient MStyleSheet styleSheet = null;
