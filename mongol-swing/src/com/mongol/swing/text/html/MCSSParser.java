...
class MCSSParser {
	...
	public static interface CSSParserCallback {
	...
	private int parseDeclaration() throws IOException {
		...
		for (int counter = unitBuffer.length() - 1; counter >= 0; counter--) {
			unitBuffer.setCharAt(counter,
					Character.toLowerCase(unitBuffer.charAt(counter)));
		}
				...
				if (tokenBufferLength > 0) {
					if (tokenBuffer[tokenBufferLength - 1] == extraChar) {
						if (--tokenBufferLength > 0) {
							...
							unitBuffer
									.append(tokenBuffer, 0, tokenBufferLength);
						}
						return IDENTIFIER;
					}
					if (readWS && unitBuffer.length() > 0) {
