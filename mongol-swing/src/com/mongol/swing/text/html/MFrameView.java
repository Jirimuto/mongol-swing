...
class MFrameView extends ComponentView implements HyperlinkListener {
		...
		if ((srcAtt != null) && (!srcAtt.equals(""))) {
			try {
				...
				if (host != null) {
					...
					MHTMLEditorKit hostKit = (MHTMLEditorKit) host.getEditorKit();
					if (hostKit != null) {
						isAutoFormSubmission = hostKit.isAutoFormSubmission();
					}
				}
				htmlPane.setPage(src);
				MHTMLEditorKit kit = (MHTMLEditorKit) htmlPane.getEditorKit();
				if (kit != null) {
	...
	public void paint(Graphics g, Shape allocation) {
		...
		if (host != null
				&& htmlPane != null
				&& htmlPane.isEditable() != ((JTextComponent) host)
						.isEditable()) {
	...
	private void setMargin() {
		...
		String marginStr = (String) attributes
				.getAttribute(MHTML.Attribute.MARGINWIDTH);
		if (in != null) {
		...
		marginStr = (String) attributes
				.getAttribute(MHTML.Attribute.MARGINHEIGHT);
		if (marginStr != null) {
			margin = Integer.parseInt(marginStr);
			if (margin > 0) {
	...
	private void setBorder() {
		...
		AttributeSet attributes = getElement().getAttributes();
		String frameBorder = (String) attributes
				.getAttribute(MHTML.Attribute.FRAMEBORDER);
		if ((frameBorder != null)
	...
	private void createScrollPane() {
		AttributeSet attributes = getElement().getAttributes();
		String scrolling = (String) attributes
				.getAttribute(MHTML.Attribute.SCROLLING);
		if (scrolling == null) {
			scrolling = "auto";
		}
	...
	JEditorPane getOutermostJEditorPane() {
		...
		MFrameSetView frameSetView = null;
		while (parent != null) {
	...
	private boolean inNestedFrameSet() {
		MFrameSetView parent = (MFrameSetView) getParent();
	...
	public void hyperlinkUpdate(HyperlinkEvent evt) {
		...
		if (e.getEventType() == HyperlinkEvent.EventType.ACTIVATED) {
			...
			if (!c.isEditable()) {
				c.fireHyperlinkUpdate(new MHTMLFrameHyperlinkEvent(c, e
	...
	private Object movePostData(JEditorPane targetPane, String frameName) {
		...
		if (p != null) {
			...
			if (frameName != null) {
				String propName = MFormView.PostDataProperty + "." + frameName;
				Document d = p.getDocument();
				postData = d.getProperty(propName);
				if (postData != null) {
					targetPane.getDocument().putProperty(
							MFormView.PostDataProperty, postData);
	 ...
	 */
	class FrameEditorPane extends JEditorPane {
