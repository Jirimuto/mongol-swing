...
public class MHTMLWriter extends AbstractWriter {
	/*
	...
	public void write() throws IOException, BadLocationException {
		...
		while ((next = it.next()) != null) {
			if (!inRange(next)) {
				if (completeDoc
						&& next.getAttributes().getAttribute(
				...
				if (indentNeedsIncrementing(current, next)) {
					...
					while (top != next.getParentElement()) {
						...
						if (!synthesizedElement(top)) {
							...
							if (!matchNameAttribute(attrs, MHTML.Tag.PRE)
			...
			if (!next.isLeaf()
					|| isFormElementWithContent(next.getAttributes())) {
	...
	protected void writeAttributes(AttributeSet attr) throws IOException {
		...
		while (names.hasMoreElements()) {
			...
			if (name instanceof MHTML.Tag || name instanceof StyleConstants
					|| name == MHTML.Attribute.ENDTAG) {
				continue;
			}
	...
	protected void emptyTag(Element elem) throws BadLocationException,
		...
		if (matchNameAttribute(attr, MHTML.Tag.CONTENT)) {
			inContent = true;
			text(elem);
		} else if (matchNameAttribute(attr, MHTML.Tag.COMMENT)) {
			comment(elem);
		} else {
			...
			Object nameTag = (attr != null) ? attr
					.getAttribute(StyleConstants.NameAttribute) : null;
			Object endTag = (attr != null) ? attr
					.getAttribute(MHTML.Attribute.ENDTAG) : null;
			...
			if (completeDoc && matchNameAttribute(attr, MHTML.Tag.HEAD)) {
				if (outputEndTag) {
					// Write out any styles.
					writeStyles(((MHTMLDocument) getDocument()).getStyleSheet());
				}
				wroteHead = true;
			}
			...
			write('>');
			if (matchNameAttribute(attr, MHTML.Tag.TITLE) && !outputEndTag) {
	...
	protected void startTag(Element elem) throws IOException,
		...
		MHTML.Tag name;
		if (nameAttribute instanceof MHTML.Tag) {
			name = (MHTML.Tag) nameAttribute;
		} else {
			name = null;
		}
		...
		if (completeDoc && name == MHTML.Tag.BODY && !wroteHead) {
		...
		} else if (completeDoc && name == MHTML.Tag.BODY) {
			// Write out the maps, which is not stored as Elements in
			// the Document.
			writeMaps(((MHTMLDocument) getDocument()).getMaps());
		} else if (name == MHTML.Tag.HEAD) {
			MHTMLDocument document = (MHTMLDocument) getDocument();
			wroteHead = true;
			incrIndent();
	...
	protected void endTag(Element elem) throws IOException {
		...
		if (matchNameAttribute(elem.getAttributes(), MHTML.Tag.PRE)) {
			inPre = false;
		}
		write('<');
		write('/');
	...
	protected void comment(Element elem) throws BadLocationException,
		...
		if (matchNameAttribute(as, MHTML.Tag.COMMENT)) {
	...
	void writeAdditionalComments() throws IOException {
		Object comments = getDocument().getProperty(
				MHTMLDocument.AdditionalComments);
	...
	protected boolean synthesizedElement(Element elem) {
		if (matchNameAttribute(elem.getAttributes(), MHTML.Tag.IMPLIED)) {
			return true;
		}
		return false;
	}
	...
	protected void writeEmbeddedTags(AttributeSet attr) throws IOException {
		...
		while (names.hasMoreElements()) {
			...
			if (name instanceof MHTML.Tag) {
				...
				if (tag == MHTML.Tag.FORM || tags.contains(tag)) {
					continue;
				}
				write('<');
				write(tag.toString());
	 ...
	 */
	private boolean noMatchForTagInAttributes(AttributeSet attr, MHTML.Tag t,
			Object tagValue) {
	...
	private boolean isFormElementWithContent(AttributeSet attr) {
		if (matchNameAttribute(attr, MHTML.Tag.TEXTAREA)
				|| matchNameAttribute(attr, MHTML.Tag.SELECT)) {
			return true;
		}
		return false;
	}
			...
			if (styles != null) {
				...
				while (styles.hasMoreElements()) {
					...
					if (!StyleContext.DEFAULT_STYLE.equals(name)
							&& writeStyle(name, sheet.getStyle(name),
									outputStyle)) {
						outputStyle = true;
					}
				}
	...
	boolean writeStyle(String name, Style style, boolean outputStyle)
		...
		if (attributes != null) {
			while (attributes.hasMoreElements()) {
				...
				if (attribute instanceof MCSS.Attribute) {
	...
	private static void convertToHTML32(AttributeSet from,
		...
		while (keys.hasMoreElements()) {
			...
			if (key instanceof MCSS.Attribute) {
				if ((key == MCSS.Attribute.FONT_FAMILY)
						|| (key == MCSS.Attribute.FONT_SIZE)
						|| (key == MCSS.Attribute.COLOR)) {

					createFontAttribute((MCSS.Attribute) key, from, to);
				} else if (key == MCSS.Attribute.FONT_WEIGHT) {
					// add a bold tag is weight is bold
					MCSS.FontWeight weightValue = (MCSS.FontWeight) from
							.getAttribute(MCSS.Attribute.FONT_WEIGHT);
					if ((weightValue != null) && (weightValue.getValue() > 400)) {
						addAttribute(to, MHTML.Tag.B, SimpleAttributeSet.EMPTY);
					}
				} else if (key == MCSS.Attribute.FONT_STYLE) {
					String s = from.getAttribute(key).toString();
					if (s.indexOf("italic") >= 0) {
						addAttribute(to, MHTML.Tag.I, SimpleAttributeSet.EMPTY);
					}
				} else if (key == MCSS.Attribute.TEXT_DECORATION) {
					String decor = from.getAttribute(key).toString();
					if (decor.indexOf("underline") >= 0) {
						addAttribute(to, MHTML.Tag.U, SimpleAttributeSet.EMPTY);
					}
					if (decor.indexOf("line-through") >= 0) {
						addAttribute(to, MHTML.Tag.STRIKE,
								SimpleAttributeSet.EMPTY);
					}
				} else if (key == MCSS.Attribute.VERTICAL_ALIGN) {
					String vAlign = from.getAttribute(key).toString();
					if (vAlign.indexOf("sup") >= 0) {
						addAttribute(to, MHTML.Tag.SUP,
								SimpleAttributeSet.EMPTY);
					}
					if (vAlign.indexOf("sub") >= 0) {
						addAttribute(to, MHTML.Tag.SUB,
								SimpleAttributeSet.EMPTY);
					}
				} else if (key == MCSS.Attribute.TEXT_ALIGN) {
					addAttribute(to, MHTML.Attribute.ALIGN,
	 ...
	 */
	private static void createFontAttribute(MCSS.Attribute a,
			AttributeSet from, MutableAttributeSet to) {
		MutableAttributeSet fontAttr = (MutableAttributeSet) to
				.getAttribute(MHTML.Tag.FONT);
		if (fontAttr == null) {
