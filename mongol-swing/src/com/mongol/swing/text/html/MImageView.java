 ...
 * @author jrmt@Almas
 */
public class MImageView extends MView {
	/**
	 ...
	 */
	public MImageView(Element elem) {
		super(elem);
	...
	public URL getImageURL() {
		...
		URL reference = ((MHTMLDocument) getDocument()).getBase();
		try {
	...
	protected void setPropertiesFromAttributes() {
		...
		// Gutters
		borderSize = (short) getIntAttr(MHTML.Attribute.BORDER,
				isLink() ? DEFAULT_BORDER : 0);

		leftInset = rightInset = (short) (getIntAttr(MHTML.Attribute.HSPACE, 0) + borderSize);
		topInset = bottomInset = (short) (getIntAttr(MHTML.Attribute.VSPACE, 0) + borderSize);
		...
		Object alignment = attr.getAttribute(MHTML.Attribute.ALIGN);
		...
		AttributeSet anchorAttr = (AttributeSet) attr.getAttribute(MHTML.Tag.A);
		if (anchorAttr != null && anchorAttr.isDefined(MHTML.Attribute.HREF)) {
			synchronized (this) {
				state |= LINK_FLAG;
			}
		} else {
	...
	public void setSize(float width, float height) {
		...
		if (getImage() == null) {
			...
			if (view != null) {
				view.setSize(Math.max(0f, width
						- (float) (DEFAULT_WIDTH + leftInset + rightInset)),
	 ...
	 * AttributeSet.
	 */
	private int getIntAttr(MHTML.Attribute name, int deflt) {
		AttributeSet attr = getElement().getAttributes();
	...
	private void updateImageSize() {
		...
		if (newImage != null) {
			...
			newWidth = getIntAttr(MHTML.Attribute.WIDTH, -1);
			if (newWidth > 0) {
				newState |= WIDTH_FLAG;
			}
			newHeight = getIntAttr(MHTML.Attribute.HEIGHT, -1);
			if (newHeight > 0) {
		...
		public boolean imageUpdate(Image img, int flags, int x, int y,
			...
			if ((flags & ImageObserver.HEIGHT) != 0
					&& !getElement().getAttributes().isDefined(
							MHTML.Attribute.HEIGHT)) {
				changed |= 1;
			}
			if ((flags & ImageObserver.WIDTH) != 0
					&& !getElement().getAttributes().isDefined(
							MHTML.Attribute.WIDTH)) {
				changed |= 2;
			}
	 ...
	 */
	private class ImageLabelView extends MInlineView {
