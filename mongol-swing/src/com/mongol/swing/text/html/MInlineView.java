 ...
 * @author jrmt@Almas
 */
public class MInlineView extends MLabelView {
	 ...
	 * @param pos
	 *            the potential location of the start of the broken view >= 0.
	 ...
	 */
	public MView breakView(int axis, int offset, float pos, float len) {
			...
			try {
				...
				for (int end = line.next(); end != BreakIterator.DONE; start = end, end = line
					...
					if (end > start) {
						span = Math.max(
								span,
								metrics.charsWidth(segment.array, start, end
										- start));
					}
				}
		...
		if (p1 > p0) {
			try {
				...
				for (int i = segment.offset; i <= lastIndex; i++) {
					...
					if (updateSpan) {
						if (end > start) {
							span = Math.max(span, metrics.charsWidth(
									segment.array, start, end - start + 1));
						}
					}
	...
	protected void setPropertiesFromAttributes() {
		...
		boolean u = (decor != null) ? (decor.toString().indexOf("underline") >= 0)
				: false;
		setUnderline(u);
		boolean s = (decor != null) ? (decor.toString().indexOf("line-through") >= 0)
				: false;
		setStrikeThrough(s);
