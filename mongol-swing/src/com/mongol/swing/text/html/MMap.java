 ...
 * @author jrmt@Almas
 */
class MMap implements Serializable {
	/** Name of the Map. */
	private String name;
	...
	public AttributeSet getArea(int x, int y, int width, int height) {
		...
		if (numAttributes > 0) {
			...
			for (int counter = 0; counter < numAttributes; counter++) {
				if (counter >= numAreas) {
					areas.addElement(createRegionContainment((AttributeSet) areaAttributes
							.elementAt(counter)));
				}
	...
	static class RectangleRegionContainment implements RegionContainment {
		...
		public RectangleRegionContainment(AttributeSet as) {
			int[] coords = MMap.extractCoords(as
					.getAttribute(MHTML.Attribute.COORDS));
