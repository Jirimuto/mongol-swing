 ...
 * @author jrmt@Almas
 */
public class MStyleSheet extends StyleContext {
	 ...
	 */
	public Style getRule(MHTML.Tag t, Element e) {
		...
		try {
			...
			// >= 1 as the MHTML.Tag for the 0th element is passed in.
			for (int counter = n - 1; counter >= 1; counter--) {
				...
				if (attr != null) {
					if (attr.isDefined(MHTML.Attribute.ID)) {
						cacheLookup.append('#');
						cacheLookup.append(attr
								.getAttribute(MHTML.Attribute.ID));
					} else if (attr.isDefined(MHTML.Attribute.CLASS)) {
						cacheLookup.append('.');
						cacheLookup.append(attr
								.getAttribute(MHTML.Attribute.CLASS));
					}
				}
				cacheLookup.append(' ');
			}
			...
			if (attr != null) {
				if (attr.isDefined(MHTML.Attribute.ID)) {
					cacheLookup.append('#');
					cacheLookup.append(attr.getAttribute(MHTML.Attribute.ID));
				} else if (attr.isDefined(MHTML.Attribute.CLASS)) {
					cacheLookup.append('.');
					cacheLookup
							.append(attr.getAttribute(MHTML.Attribute.CLASS));
				}
			}
	 ...
	 * @since 1.3
	 */
	public void addStyleSheet(MStyleSheet ss) {
		synchronized (this) {
	 ...
	 * @since 1.3
	 */
	public void removeStyleSheet(MStyleSheet ss) {
		synchronized (this) {
			if (linkedStyleSheets != null) {
	 ...
	 * @since 1.3
	 */
	public MStyleSheet[] getStyleSheets() {
		MStyleSheet[] retValue;

		synchronized (this) {
			if (linkedStyleSheets != null) {
				retValue = new MStyleSheet[linkedStyleSheets.size()];
	 ...
	 * @since 1.3
	 */
	public void addCSSAttribute(MutableAttributeSet attr, MCSS.Attribute key,
			String value) {
	...
	public AttributeSet addAttribute(AttributeSet old, Object key, Object value) {
		...
		if (key instanceof StyleConstants) {
			MHTML.Tag tag = MHTML
					.getTagForStyleConstantsKey((StyleConstants) key);
			...
			if (cssValue != null) {
				Object cssKey = css
						.styleConstantsKeyToCSSKey((StyleConstants) key);
				if (cssKey != null) {
	...
	public AttributeSet addAttributes(AttributeSet old, AttributeSet attr) {
		if (!(attr instanceof MHTMLDocument.TaggedAttributeSet)) {
	...
	/**
	 * For any StyleConstants key in attr that has an associated MHTML.Tag, it
	 * is removed from old. The resulting AttributeSet is then returned.
	 */
	...
	public Color getForeground(AttributeSet a) {
		Color c = css.getColor(a, MCSS.Attribute.COLOR);
		if (c == null) {
			return Color.black;
		}
		return c;
	...
	public Color getBackground(AttributeSet a) {
		return css.getColor(a, MCSS.Attribute.BACKGROUND_COLOR);
	}
	 ...
	 * identifiy a relative font size such as +1 or -2.
	 */
	...
	public static int getIndexOfSize(float pt) {
		return MCSS.getIndexOfSize(pt, sizeMapDefault);
	}
	...
	public Color stringToColor(String string) {
		return MCSS.stringToColor(string);
	}
	...
	ImageIcon getBackgroundImage(AttributeSet attr) {
		Object value = attr.getAttribute(MCSS.Attribute.BACKGROUND_IMAGE);

		if (value != null) {
			return ((MCSS.BackgroundImage) value).getImage(getBase());
		}
		return null;
	}
	...
	void addRule(String[] selector, AttributeSet declaration, boolean isLinked) {
		...
		if (rule == null) {
			...
			synchronized (this) {
				...
				for (int i = n - 1; i >= 0; i--) {
					mapping = mapping
							.getChildSelectorMapping(selector[i], true);
				}
	 ...
	 * <code>ss</code>.
	 */
	private synchronized void linkStyleSheetAt(MStyleSheet ss, int index) {
		if (resolvedStyles.size() > 0) {
			Enumeration values = resolvedStyles.elements();
			while (values.hasMoreElements()) {
	 ...
	 */
	private synchronized void unlinkStyleSheet(MStyleSheet ss, int index) {
		if (resolvedStyles.size() > 0) {
			Enumeration values = resolvedStyles.elements();
			while (values.hasMoreElements()) {
				ResolvedStyle rule = (ResolvedStyle) values.nextElement();
	...
	/**
	 * Returns a string that only has one space between simple selectors, which
	 * may be the passed in String.
	 */
	...
	private void addSortedStyle(SelectorMapping mapping, Vector elements) {
		...
		if (size > 0) {
			...
			for (int counter = 0; counter < size; counter++) {
				if (specificity >= ((SelectorMapping) elements
						.elementAt(counter)).getSpecificity()) {
	...
	private synchronized void getStyles(SelectorMapping parentMapping,
			Vector styles, String[] tags, String[] ids, String[] classes,
			int index, int numElements, Hashtable alreadyChecked) {
	...
	private synchronized Style createResolvedStyle(String selector,
		...
		try {
			...
			for (int counter = 0; counter < numLinkedSS; counter++) {
				AttributeSet attr = ((MStyleSheet) linkedStyleSheets
	...
	private Style createResolvedStyle(String selector, Vector elements,
		...
		for (int counter = 0; counter < numElements; counter++) {
			...
			if (attr != null) {
				...
				if (attr.isDefined(MHTML.Attribute.CLASS)) {
					classes[counter] = attr.getAttribute(MHTML.Attribute.CLASS)
							.toString();
				} else {
					classes[counter] = null;
				}
				if (attr.isDefined(MHTML.Attribute.ID)) {
					ids[counter] = attr.getAttribute(MHTML.Attribute.ID)
							.toString();
				} else {
					ids[counter] = null;
				}
	...
	public static class BoxPainter implements Serializable {
		...
		BoxPainter(AttributeSet a, MCSS css, MStyleSheet ss) {
			this.ss = ss;
			this.css = css;
			border = getBorder(a);
			binsets = border.getBorderInsets(null);
			topMargin = getLength(MCSS.Attribute.MARGIN_TOP, a);
			bottomMargin = getLength(MCSS.Attribute.MARGIN_BOTTOM, a);
			leftMargin = getLength(MCSS.Attribute.MARGIN_LEFT, a);
			rightMargin = getLength(MCSS.Attribute.MARGIN_RIGHT, a);
		...
		Border getBorder(AttributeSet a) {
			Border b = noBorder;
			Object o = a.getAttribute(MCSS.Attribute.BORDER_STYLE);
			if (o != null) {
				String bstyle = o.toString();
				int bw = (int) getLength(MCSS.Attribute.BORDER_TOP_WIDTH, a);
				if (bw > 0) {
					if (bstyle.equals("inset")) {
						Color c = getBorderColor(a);
						b = new BevelBorder(BevelBorder.LOWERED, c.brighter(),
								c.darker());
					} else if (bstyle.equals("outset")) {
						Color c = getBorderColor(a);
						b = new BevelBorder(BevelBorder.RAISED, c.brighter(),
								c.darker());
		...
		Color getBorderColor(AttributeSet a) {
			Color color = css.getColor(a, MCSS.Attribute.BORDER_COLOR);
			if (color == null) {
				color = css.getColor(a, MCSS.Attribute.COLOR);
				if (color == null) {
					return Color.black;
				}
			}
		...
		public float getInset(int side, View v) {
			...
			case View.LEFT:
				...
				inset += binsets.left;
				inset += getLength(MCSS.Attribute.PADDING_LEFT, a);
				break;
			case View.RIGHT:
				inset += getOrientationMargin(HorizontalMargin.RIGHT,
						rightMargin, a, isLeftToRight(v));
				inset += binsets.right;
				inset += getLength(MCSS.Attribute.PADDING_RIGHT, a);
				break;
			case View.TOP:
				inset += topMargin;
				inset += binsets.top;
				inset += getLength(MCSS.Attribute.PADDING_TOP, a);
				break;
			case View.BOTTOM:
				inset += bottomMargin;
				inset += binsets.bottom;
				inset += getLength(MCSS.Attribute.PADDING_BOTTOM, a);
				break;
			default:
		...
		public void paint(Graphics g, float x, float y, float w, float h, View v) {
			...
			if (!(v instanceof MHTMLEditorKit.MHTMLFactory.MBodyBlockView)) {
				dx = localLeftMargin;
				dy = topMargin;
		...
		float getLength(MCSS.Attribute key, AttributeSet a) {
		...
		static boolean isOrientationAware(View v) {
			...
			if (v != null
					...
					&& (obj = attr.getAttribute(StyleConstants.NameAttribute)) instanceof MHTML.Tag
					&& (obj == MHTML.Tag.DIR || obj == MHTML.Tag.MENU
							|| obj == MHTML.Tag.UL || obj == MHTML.Tag.OL)) {
				ret = true;
			}
		...
		float getOrientationMargin(HorizontalMargin side, float cssMargin,
			...
			case RIGHT: {
				orientationMargin = (isLeftToRight) ? getLength(
						MCSS.Attribute.MARGIN_RIGHT_LTR, a) : getLength(
						MCSS.Attribute.MARGIN_RIGHT_RTL, a);
				cssMarginValue = a.getAttribute(MCSS.Attribute.MARGIN_RIGHT);
			}
				break;
			case LEFT: {
				orientationMargin = (isLeftToRight) ? getLength(
						MCSS.Attribute.MARGIN_LEFT_LTR, a) : getLength(
						MCSS.Attribute.MARGIN_LEFT_RTL, a);
				cssMarginValue = a.getAttribute(MCSS.Attribute.MARGIN_LEFT);
			}
				break;
			}
	...
	public static class ListPainter implements Serializable {
		...
		ListPainter(AttributeSet attr, MStyleSheet ss) {
			this.ss = ss;
			/* Get the image to use as a list bullet */
			String imgstr = (String) attr
					.getAttribute(MCSS.Attribute.LIST_STYLE_IMAGE);
			type = null;
			...
			if (img == null) {
				type = (MCSS.Value) attr
						.getAttribute(MCSS.Attribute.LIST_STYLE_TYPE);
			}
			start = 1;
		...
		/**
		 * Returns a string that represents the value of the
		 * MHTML.Attribute.TYPE attribute. If this attributes is not defined,
		 * then then the type defaults to "disc" unless the tag is on Ordered
		 * list. In the case of the latter, the default type is "decimal".
		 */
		private MCSS.Value getChildType(View childView) {
			MCSS.Value childtype = (MCSS.Value) childView.getAttributes()
					.getAttribute(MCSS.Attribute.LIST_STYLE_TYPE);
			...
			if (childtype == null) {
				if (type == null) {
					...
					MHTMLDocument doc = (MHTMLDocument) v.getDocument();
					if (doc.matchNameAttribute(v.getElement().getAttributes(),
							MHTML.Tag.OL)) {
						childtype = MCSS.Value.DECIMAL;
					} else {
						childtype = MCSS.Value.DISC;
					}
				} else {
					childtype = type;
				}
		...
		private void getStart(View parent) {
			...
			if (element != null) {
				...
				Object startValue;
				if (attr != null
						&& attr.isDefined(MHTML.Attribute.START)
						&& (startValue = attr
								.getAttribute(MHTML.Attribute.START)) != null
		 ...
		 * MHTML.Attribute.START specified.
		 */
		private int getRenderIndex(View parentView, int childIndex) {
			...
			for (int counter = childIndex; counter >= 0; counter--) {
				AttributeSet as = parentView.getElement().getElement(counter)
						.getAttributes();
				if (as.getAttribute(StyleConstants.NameAttribute) != MHTML.Tag.LI) {
					retIndex--;
				} else if (as.isDefined(MHTML.Attribute.VALUE)) {
					Object value = as.getAttribute(MHTML.Attribute.VALUE);
		...
		public void paint(Graphics g, float x, float y, float w, float h,
			...
			if (!(name instanceof MHTML.Tag) || name != MHTML.Tag.LI) {
				return;
			}
			// deside on what side draw bullets, etc.
			isLeftToRight = cv.getContainer().getComponentOrientation()
					.isLeftToRight();
			...
			MCSS.Value childtype = getChildType(cv);
			Font font = ((StyledDocument) cv.getDocument()).getFont(cv
					.getAttributes());
			if (font != null) {
				g.setFont(font);
			}
			if (childtype == MCSS.Value.SQUARE || childtype == MCSS.Value.CIRCLE
					|| childtype == MCSS.Value.DISC) {
				drawShape(g, childtype, (int) x, (int) y, (int) w, (int) h,
						align);
			} else if (childtype == MCSS.Value.DECIMAL) {
				drawLetter(g, '1', (int) x, (int) y, (int) w, (int) h, align,
						getRenderIndex(v, item));
			} else if (childtype == MCSS.Value.LOWER_ALPHA) {
				drawLetter(g, 'a', (int) x, (int) y, (int) w, (int) h, align,
						getRenderIndex(v, item));
			} else if (childtype == MCSS.Value.UPPER_ALPHA) {
				drawLetter(g, 'A', (int) x, (int) y, (int) w, (int) h, align,
						getRenderIndex(v, item));
			} else if (childtype == MCSS.Value.LOWER_ROMAN) {
				drawLetter(g, 'i', (int) x, (int) y, (int) w, (int) h, align,
						getRenderIndex(v, item));
			} else if (childtype == MCSS.Value.UPPER_ROMAN) {
				drawLetter(g, 'I', (int) x, (int) y, (int) w, (int) h, align,
						getRenderIndex(v, item));
			}
		}
		 ...
		 *            preferred alignment factor for the child view
		 */
		void drawShape(Graphics g, MCSS.Value type, int ax, int ay, int aw,
				int ah, float align) {
			...
			if (type == MCSS.Value.SQUARE) {
				g.drawRect(x, y, 8, 8);
			} else if (type == MCSS.Value.CIRCLE) {
				g.drawOval(x, y, 8, 8);
			} else {
				g.fillOval(x, y, 8, 8);
			}
		...
		static final char romanChars[][] = { { 'i', 'v' }, { 'x', 'l' },
				{ 'c', 'd' }, { 'm', '?' }, };
		...
		private int start;
		private MCSS.Value type;
		URL imageurl;
		private MStyleSheet ss = null;
		Icon img = null;
	...
	static class BackgroundImagePainter implements Serializable {
		...
		BackgroundImagePainter(AttributeSet a, MCSS css, MStyleSheet ss) {
			backgroundImage = ss.getBackgroundImage(a);
			// Determine the position.
			MCSS.BackgroundPosition pos = (MCSS.BackgroundPosition) a
					.getAttribute(MCSS.Attribute.BACKGROUND_POSITION);
			if (pos != null) {
			...
			MCSS.Value repeats = (MCSS.Value) a
					.getAttribute(MCSS.Attribute.BACKGROUND_REPEAT);
			if (repeats == null || repeats == MCSS.Value.BACKGROUND_REPEAT) {
				flags |= 3;
			} else if (repeats == MCSS.Value.BACKGROUND_REPEAT_X) {
				flags |= 1;
			} else if (repeats == MCSS.Value.BACKGROUND_REPEAT_Y) {
				flags |= 2;
			}
		}
			...
			if ((flags & 3) == 0) {
				...
				if (clip == null
						|| !((paintX + width <= clip.x)
								...
								|| (paintX >= clip.x + clip.width) || (paintY >= clip.y
								+ clip.height))) {
		...
		private boolean updatePaintCoordinates(Rectangle clip, int width,
			...
			if (clip != null) {
				if ((flags & 3) == 1
						&& ((paintY + height <= clip.y) || (paintY > clip.y
								+ clip.height))) {
					// not visible.
					return false;
				}
				if ((flags & 3) == 2
						&& ((paintX + width <= clip.x) || (paintX > clip.x
								+ clip.width))) {
					// not visible.
					return false;
				}
				if ((flags & 1) == 1) {
					if ((clip.x + clip.width) < paintMaxX) {
						if ((clip.x + clip.width - paintX) % width == 0) {
							...
							paintMaxX = ((clip.x + clip.width - paintX) / width + 1)
									* width + paintX;
						}
					}
					if (clip.x > paintX) {
	 ...
	 * Views Elements.
	 */
	class ViewAttributeSet extends MMuxingAttributeSet {
		ViewAttributeSet(View v) {
			...
			try {
				if (doc instanceof MHTMLDocument) {
					MStyleSheet styles = MStyleSheet.this;
					...
					if (elem.isLeaf()) {
						...
						while (keys.hasMoreElements()) {
							...
							if (key instanceof MHTML.Tag) {
								if ((MHTML.Tag) key == MHTML.Tag.A) {
									Object o = a.getAttribute((MHTML.Tag) key);
									/**
									...
									if (o != null && o instanceof AttributeSet) {
										...
										if (attr.getAttribute(MHTML.Attribute.HREF) == null) {
											continue;
										}
									}
								}
		...
		public Object getAttribute(Object key) {
			if (key instanceof StyleConstants) {
				...
				if (cssKey != null) {
					...
					if (value instanceof MCSS.CssValue) {
						return ((MCSS.CssValue) value).toStyleConstants(
		...
		Object doGetAttribute(Object key) {
			...
			if (key instanceof MCSS.Attribute) {
				MCSS.Attribute css = (MCSS.Attribute) key;
				if (css.isInherited()) {
	...
	// implement Style.
	public static class ResolvedStyle extends MMuxingAttributeSet implements
			Serializable, Style {
		...
		boolean matches(String selector, int sCurrent, int sLast,
			...
			if (thisDotIndex != -1) {
				...
				return (((thisDotIndex - thisCurrent) == (sLast - sCurrent)) && selector
						.regionMatches(sCurrent, name, thisCurrent,
		...
		/**
		 * Similiar to String.indexOf, but allows an upper bound (this is slower
	 ...
	 * middle of parsing.
	 */
	class CssParser implements MCSSParser.CSSParserCallback {
		...
		public void handleImport(String importString) {
			URL url = MCSS.getURL(base, importString);
			if (url != null) {
		...
		public void handleValue(String value) {
			if (propertyName != null && value != null && value.length() > 0) {
				MCSS.Attribute cssKey = MCSS.getAttribute(propertyName);
				if (cssKey != null) {
					...
					if (cssKey == MCSS.Attribute.LIST_STYLE_IMAGE) {
						if (value != null && !value.equals("none")) {
							URL url = MCSS.getURL(base, value);
		...
		public void endRule() {
			...
			for (int i = 0; i < n; i++) {
				...
				if (selector.length > 0) {
					MStyleSheet.this.addRule(selector, declaration, isLink);
				}
			}
			declaration.removeAttributes(declaration);
			selectors.removeAllElements();
		...
		URL base;
		MCSSParser parser = new MCSSParser();
	}
	...
	void rebaseSizeMap(int base) {
		...
		for (int i = 0; i < sizeMapDefault.length; i++) {
			sizeMap[i] = Math.max(base * sizeMapDefault[i]
					/ sizeMapDefault[MCSS.baseFontSizeIndex], minimalFontSize);
		}
	 ...
	 */
	public static final int sizeMapDefault[] = { 8, 10, 12, 14, 18, 24, 36 };
