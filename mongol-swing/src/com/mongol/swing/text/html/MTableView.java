...
/**
 * Modified HTML table view.
 * 
 * @author jrmt@Almas
 */
public class MTableView extends MBoxView implements ViewFactory {
	...
	protected int getColumnsOccupied(View v) {
		...
		if (a.isDefined(MHTML.Attribute.COLSPAN)) {
			String s = (String) a.getAttribute(MHTML.Attribute.COLSPAN);
			if (s != null) {
				try {
	...
	protected int getRowsOccupied(View v) {
		...
		if (a.isDefined(MHTML.Attribute.ROWSPAN)) {
			String s = (String) a.getAttribute(MHTML.Attribute.ROWSPAN);
			if (s != null) {
				try {
					return Integer.parseInt(s);
				} catch (NumberFormatException nfe) {
	...
	protected void setPropertiesFromAttributes() {
		...
		if (attr != null) {
			...
			MCSS.LengthValue lv = (MCSS.LengthValue) attr
					.getAttribute(MCSS.Attribute.BORDER_SPACING);
			if (lv != null) {
				cellSpacing = (int) lv.getValue();
			} else {
				cellSpacing = 0;
			}
			lv = (MCSS.LengthValue) attr
					.getAttribute(MCSS.Attribute.BORDER_TOP_WIDTH);
			if (lv != null) {
			...
			for (int row = 0; row < nrows; row++) {
				...
				for (int cell = 0; cell < rv.getViewCount(); cell++, col++) {
					...
					if (!relativeCells) {
						...
						MCSS.LengthValue lv = (MCSS.LengthValue) a
								.getAttribute(MCSS.Attribute.WIDTH);
	...
	protected void layoutColumns(int targetSpan, int[] offsets, int[] spans,
		...
		MCSS.calculateTiledLayout(colIterator, targetSpan);
	}
	...
	protected SizeRequirements calculateMinorAxisRequirements(int axis,
		...
		MCSS.LengthValue cssWidth = (MCSS.LengthValue) attr
				.getAttribute(MCSS.Attribute.WIDTH);

		if (MBlockView.spanSetFromAttributes(axis, r, cssWidth, null)) {
	...
	protected SizeRequirements calculateMajorAxisRequirements(int axis,
		...
		r = MCSS.calculateTiledRequirements(rowIterator, r);
	...
	protected void layoutMajorAxis(int targetSpan, int axis, int[] offsets,
		...
		MCSS.calculateTiledLayout(rowIterator, targetSpan);
	 ...
	 *         one
	 */
	protected MView getViewAtPosition(int pos, Rectangle a) {
		int n = getViewCount();
		for (int i = 0; i < n; i++) {
			MView v = getView(i);
	...
	public View create(Element elem) {
		...
		if (o instanceof MHTML.Tag) {
			...
			} else if ((kind == MHTML.Tag.TD) || (kind == MHTML.Tag.TH)) {
				return new CellView(elem);
			} else if (kind == MHTML.Tag.CAPTION) {
	...
	class ColumnIterator implements MCSS.LayoutIterator {
	...
	class RowIterator implements MCSS.LayoutIterator {
		...
		public float getPreferredSpan(float parentSpan) {
			...
			if (rv != null) {
				...
				return rv.getPreferredSpan(MTableView.this.getAxis()) + adjust;
			}
			return 0;
		}
	 ...
	 */
	public class RowView extends MBoxView {
		...
		MView findViewAtPoint(int x, int y, Rectangle alloc) {
			int n = getViewCount();
			for (int i = 0; i < n; i++) {
	 ...
	 * else.
	 */
	class CellView extends MBlockView {
		...
		protected void layoutMajorAxis(int targetSpan, int axis, int[] offsets,
			...
			if (used < targetSpan) {
				...
				String valign = (String) getElement().getAttributes()
						.getAttribute(MHTML.Attribute.VALIGN);
				if (valign == null) {
